...
export async function registerEmailJob() {
    ...
    await boss.work(QUEUE, async (job) => {
        ...
        if(job[0].data.templateName == "mailAttendes"){
            ...
            sendEmail(job[0].data.to, eventUpdates(job[0].data.eventUpdates, job[0].data.user));
        } else if (job[0].data.templateName == "cancelEvent"){
            sendEmail(job[0].data.to, cancelEvent(job[0].data.events, job[0].data.reason, job[0].data.user))
